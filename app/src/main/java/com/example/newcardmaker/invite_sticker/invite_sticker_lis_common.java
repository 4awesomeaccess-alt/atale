    ...
    public class invite_sticker_lis_common extends Fragment implements invite_sticker_main_category.Searchable {
        ...
        private String sticker_cidddddd = "";
        ...
        public invite_sticker_lis_common() {
        }

        public invite_sticker_lis_common(String id) {
            this.sticker_cidddddd = id;
        }

        // ════════════════════════════════════════════════════════════════
        //  Searchable interface — Activity thi call thase
        // ════════════════════════════════════════════════════════════════
        ...
        private class SwipeGestureListener extends GestureDetector.SimpleOnGestureListener {
            ...
            public boolean onFling(MotionEvent e1, MotionEvent e2,
                ...
                if (Math.abs(diffX) <= SWIPE_THRESHOLD ||
                        Math.abs(velocityX) <= SWIPE_VELOCITY_THRESHOLD) return false;

                Fragment parent = getParentFragment();
                if (!(parent instanceof invite_sticker_fragment_common)) return false;

                invite_sticker_fragment_common parentFragment =
                        (invite_sticker_fragment_common) parent;
        ...
        private void initview1() {
            try {
                ...
                sticker_lLayout = new StaggeredGridLayoutManager(3,
                ...
                GestureDetector gestureDetector =
                        new GestureDetector(getActivity(), new SwipeGestureListener());
                ...
                // Infinite scroll
                sticker_recyclerView.addOnScrollListener(
                        new invite_EndlessRecyclerViewScrollListener1(sticker_lLayout) {
                            @Override
                                ...
                                new invite_AppConstants.RecyclerTouchListener.ClickListener() {
                                    ...
                                    public void onClick(View view, int position) {
                                      ...
                                      //  returnIntent.putExtra("image",sticker_arrayList.get(position).getImageBig());
                                        returnIntent.putExtra("image", sticker_arrayList.get(position).getImageBig());
                                        returnIntent.putExtra("catid", sticker_cidddddd);
                                        returnIntent.putExtra("getAd_on_off",
                                                sticker_arrayList.get(position).getAd_on_off());

                                        Fragment parent = getParentFragment();
                                        String categoryName = "";

                                        if (parent instanceof invite_sticker_fragment_common) {
                                            invite_sticker_fragment_common parentFragment =
                                                    (invite_sticker_fragment_common) parent;
                                            int currentPosition =
                                                    parentFragment.getSelectedPosition();
                                            ArrayList<invite_ItemSubCat> categoryList =
                                                    parentFragment.getCategoryList();
                                            if (categoryList != null
                                                    && currentPosition >= 0
                                                    && currentPosition < categoryList.size()) {
                                                categoryName =
                                                        categoryList.get(currentPosition).getName();
                                            }
                                        }
           ...
           private void loadQuotesByCat(String methodQuotesCatImage) {
            ...
            if (sticker_methods == null) return;

            if (sticker_methods.isConnectingToInternet()) {

                // Search hoy to alag method, normal hoy to original method
                String apiMethod = currentSearchQuery.trim().isEmpty()
                        ? methodQuotesCatImage
                        : invite_AppConstants.METHOD_SEARCH_IMAGE_sticker;

                // Search hoy to catId blank, normal hoy to original catId
                String catId = currentSearchQuery.trim().isEmpty()
                        ? sticker_cidddddd
                        : "";

                Log.println(ASSERT,"currentSearchQuery",currentSearchQuery+"");
                requestBody = sticker_methods.getAPIRequest(
                        apiMethod,
                        sticker_page,
                        "",
                        currentSearchQuery, // p4 — search query
                        "",
                        catId,              // p6 — search ma blank, normal ma catId
                        "",
                        "",
                        "",
                        "",
                ...
                loadQuotes = new invite_Load_OneImages_dialog(
                        new invite_OneImagesListener_dialog() {
                            ...
                            public void onEnd(String success, String verifyStatus,
                                              String message,
                                              ArrayList<invite_Item_OneImages_dialog> arrayListQuotes,
                                              int total_records) {
                                if (!isAdded()) return;

                                if ("1".equals(success)) {
                                    if (!"-1".equals(verifyStatus) &&
                                            !"-2".equals(verifyStatus)) {

                                        if (arrayListQuotes == null ||
                                                arrayListQuotes.size() == 0) {
                                            sticker_isOver = true;
                                            setEmpty(true,
                                                    getString(R.string.err_no_quotes_found));
                                        } else {
                                        ...
                                        if (sticker_methods != null)
                                            sticker_methods.getVerifyDialog(
                                                    getString(R.string.error_unauth_access),
                                                    message);
                                    }
                                } else {
        ...
        private void setAdapter() {
            ...
            if (!sticker_isScroll) {
                sticker_adapterImageQuotes =
                        new invite_Adapter_Image_Greeting_dialog_special(
                                getActivity(), sticker_arrayList);
                if (sticker_recyclerView != null)
